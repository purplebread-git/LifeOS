"""SimpleContextBuilder — реализация ContextBuilder с опциональной инъекцией памяти.
...
При наличии MemoryProvider ищет релевантные воспоминания по последнему
USER-сообщению и добавляет их в system prompt.
"""
...
# Сейчас никто не записывает это значение; builder читает его, если есть.
...
class SimpleContextBuilder(ContextBuilder):
    ...
    async def build(self, conversation: Conversation) -> list[Message]:
        context = conversation.messages.copy()

        memories = await self._resolve_memory_entries(conversation)
        if not memories:
            return context

        return [self._memory_system_message(memories), *context]

...
плагин. Typed `register_*` остаётся осознанным дублированием. PluginLoader и
generic registry по-прежнему не вводятся: трёх осей достаточно, чтобы
подтвердить закономерность composition root, но недостаточно, чтобы оправдать
новый слой инфраструктуры без отдельной боли.

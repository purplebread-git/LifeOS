...
Инвариант: **Streaming — это альтернативный транспорт результата, а не
альтернативный режим выполнения агента.**

Сильнее: **ConversationEngine имеет единственную семантику выполнения.
Различаются только способы выдачи результата.**
...
```
   ...
   run_turn()         stream_turn()
        │                   │
   full response       text stream
```

Не два движка и не две реализации — один execution model.

* `LLMProvider.generate` и `LLMProvider.stream` сосуществуют;
* acceptance (текст): `''.join(stream) == generate().text`;
* `stream_turn` с tools: тот же ReAct, что `run_turn`; наружу только текст,
  tool execution — тихая пауза (без tool_start / tool_result events);
* HTTP: `POST /v1/chat/stream` — SSE, только `data:` с текстом (+ `[DONE]`);
  без WebSocket / heartbeat / typed events;
* cancellation / observability — отдельные PR.
...
* LLM Streaming MVP ✅ — `LLMProvider.stream` + `ConversationEngine.stream_turn`.
  Инвариант: Streaming — альтернативный транспорт, не альтернативный режим
  выполнения (`''.join(stream) == generate().text`).
